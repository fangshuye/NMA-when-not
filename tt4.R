...
log_odds <- function(p1,p2){
  odds1 <- p1/(1-p1)
  odds2 <- p2/(1-p2)
  log(odds1/odds2)
}

stat_information <- function(effect_size = 0.1, alpha = 0.05, beta = 0.1, num_trt = 3){
  n <- choose(num_trt, 2)
  upper <- qnorm(1-beta)-qnorm(alpha/(2*n))
  upper/effect_size
}

stat_information(effect_size = 0.28) # 13.24 vs 13.13
stat_information(effect_size = 0.10) # 38.56 vs 36.76
stat_information(effect_size = 0.18) # 20.16 vs 20.41

stat_information(num_trt = 2) # 36.76 vs 32.42
...
# CEFTH and TRIM
BRD_subset <- BRD %>%
  filter(Arm.1 == 'Ceftiofur hydrochloride' | 
           Arm.1 == 'Trimethoprim' |
           Arm.2 == 'Ceftiofur hydrochloride' | 
           Arm.2 == 'Trimethoprim' )
